...
const BASE_URL = "https://uas1eff9gh.execute-api.us-east-1.amazonaws.com/dev";
...
function drawLinks(links) {
  ...
  linkPointer
    .filter(function(d) {
      return d.source.type === INTERMEDIATE || d.source === d.target;
    })
...
function tick() {
  // Update SVG size
  updateSvgSize();
  svgPointer.attr("height", height).attr("width", width);
...
$(document).ready(function() {
  ...
  const editors = {
    specEditor: CodeMirror.fromTextArea(
      document.getElementById("goldbar-input"),
      {
        lineNumbers: true,
        lineWrapping: true
      }
  ...
  async function processSBOL(file) {
    ...
    $.ajax({
      // url: 'https://uas1eff9gh.execute-api.us-east-1.amazonaws.com/dev/importSBOL',
      url: `${BASE_URL}/importSBOL`,
      type: "POST",
      data: sbolXML,
  ...
  $("#debug-option").on("click", function() {
    ...
    editors.specEditor.setValue(
      "one-or-more (promoter or ribosomeBindingSite) then (zero-or-more cds) then terminator"
    );
    editors.catEditor.setValue(
      '{"promoter": ["BBa_R0040", "BBa_J23100"],\n "ribosomeBindingSite": ["BBa_B0032", "BBa_B0034"], \n"cds": ["BBa_E0040", "BBa_E1010"],\n"nonCodingRna": ["BBa_F0010"],\n"terminator": ["BBa_B0010"]}'
    );
  ...
  $("#submitBtn").click(function() {
    ...
    designName = designName.replace(/[^A-Z0-9]/gi, "_");

    // $.post('https://uas1eff9gh.execute-api.us-east-1.amazonaws.com/dev/postSpecs', {
    $.post(
      `${BASE_URL}/postSpecs`,
      {
...
function downloadSBOL(text, filename) {
  ...
  element.setAttribute(
    "href",
    "data:application/xml," + encodeURIComponent(text)
  );
